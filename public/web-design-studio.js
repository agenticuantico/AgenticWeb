...
run.addEventListener("click",async()=>{
 ...
 run.disabled=true; status.textContent="BUILD · diseñando con IA…";
 try{
  ...
  frame.srcdoc=d.html; code.value=d.html; status.textContent="BUILD · diseño generado · 3D activo";
 }catch(e){status.textContent="Diseñador no disponible · "+e.message}
 finally{run.disabled=false}
});
document.querySelectorAll("[data-design-prompt], [data-build-prompt]").forEach(b=>b.addEventListener("click",()=>{prompt.value=b.dataset.designPrompt;run.click()}));
})();